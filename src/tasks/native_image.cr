...
class Hokusai::Native::Tasks::NativeImage < Barista::Task
  ...
  def build : Nil
    ...
    if macos?
      ...
      command("gcc  -L. -L#{config.directory}/raylib/src -I#{config.directory}/raylib/src -I. -o hokusai-backend hokusai-backend.c hashmap.c #{config.directory}/raylib/libraylib.a -Wl,-rpath,'$ORIGIN' -lhokusai-native -lGL -lm -lpthread -ldl -lrt -lX11", chdir: native_compile_dir, env: env)
    end
